...
export class DoubaoClient {
  ...
  async generateTextToVideo(request: {
    ...
    // Use the latest Doubao 1.5 Pro model for video generation
    const response = await fetch(this.baseUrl, {
      ...
      body: JSON.stringify({
        model: 'doubao-1.5-pro-seedance', // Updated to use Doubao 1.5 Pro
        content: [
          {
            type: 'text',
            text: `${request.prompt} --resolution ${request.resolution || '1080p'} --duration ${request.duration || '5'} --aspect_ratio ${request.aspect_ratio || '16:9'} --camerafixed false --watermark true`
          }
        ],
        // Enhanced parameters for Doubao 1.5 Pro
        parameters: {
          temperature: 0.7,
          max_tokens: 4096,
          top_p: 0.9
  ...
  async generateImageToVideo(request: {
    ...
    // Use the latest Doubao 1.5 Pro model for image-to-video generation
    const response = await fetch(this.baseUrl, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.apiKey}`,
      ...
      body: JSON.stringify({
        ...
        content: [
          {
            type: 'text',
            text: `${request.prompt} --resolution ${request.resolution || '1080p'} --duration ${request.duration || '5'} --camerafixed false --watermark true`
          },
          {
            type: 'image_url',
            image_url: {
