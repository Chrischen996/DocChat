...
export class DoubaoClient {
    ...
    const response = await fetch(this.baseUrl, {
      ...
      body: JSON.stringify({
        ...
        content: [
          {
            type: 'text',
            text: `${textContent} --resolution ${request.resolution || '720p'} --duration ${request.duration || '5'} --camerafixed ${request.camera_fixed !== undefined ? request.camera_fixed : false} --watermark ${request.watermark !== undefined ? request.watermark : true}`
          }
        ],
      }),
    });
  ...
  private convertToStandardResponse(doubaoResponse: any, prompt: string): DoubaoApiResponse {
    ...
    return {
      video: {
        url: doubaoResponse.result?.video_url || doubaoResponse.data?.video_url || doubaoResponse.video_url || '',
