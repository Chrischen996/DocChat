...
def _classify_state(state: AgentState) -> AgentState:
    template = _find_template(state.template_id, state.user_input)
    if template:
        state.template_id = template["id"]
        state.workflow_id = template.get("workflow_id")
    else:
        state.workflow_id = state.workflow_id or "assistant_plan"
...
def _document_node(state: AgentState) -> AgentState:
    ...
    result = query_documents(state.user_input, history=state.messages)
...
def _image_node(state: AgentState) -> AgentState:
    ...
    state.asset = add_asset(
        ...
        format="png",
        source_template_id=state.template_id,
...
def _assistant_node(state: AgentState) -> AgentState:
    ...
    state.answer = chat_with_assistant(state.user_input, history=state.messages)
    return state
